...
{
   public class Sangat_EnterPrise 
    {
        ...
        public void ExtractPDFData(ViewPdfModel pdfViewModel, string filePath, string ProcessedInvoicePath)
        {
            ...
            try
            {
               // long firstProcessInstanceId = tblProcessInstanceData.ProcessInstanceId;
                ...
                int invIndex = pdf.FindIndex(x => x.Equals("Invoice No. Dated"));
                var invarr = pdf[invIndex + 2].Split(' ');
                pdfObject.InvoiceDate = invarr[invarr.Length - 1];
                pdfObject.InvoiceNumber = invarr[invarr.Length - 2];
                ...
                pdfObject.PONumber = pdf[pdf.FindIndex(x => x.Contains("Buyer’s Order No.")) + 2].Trim().Split(' ')[0];
        ...
        public static List<ItemList> ListofGoods(ViewPdfModel model)
        {
            ...
            foreach (var lineintextpdf in textpdf)
            {
                if (lineintextpdf.ToUpper().Contains("PCS"))
                {
                    ...
                    if (!(lineintextpdf.Replace(".","").ToUpper().Split(new[] { "PCS" }, StringSplitOptions.None)[1].Trim().Equals("")))
                    {
        ...
        public static List<string> GetDescription(List<string> pdflist, List<int> indexes)
        {
            ...
            for (int i = 0; i < indexes.Count; i++)
            {
                ...
                {
                    desc = string.Empty;
                     nameofGoods = pdflist[indexes[i]].ToUpper().Split(new[] { "PCS" }, StringSplitOptions.None)[0];
