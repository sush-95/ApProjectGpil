...
{
    public class SIFY_TECHNOLOGIES_LTD : IPDFProcessor
    {
        ...
        public SIFY_TECHNOLOGIES_LTD(IGSTDBManager dbManager, TBL_ProcessInstances tblProcessInstance)
        {
        ...
        public void ExtractPDFData(ViewPdfModel pdfViewModel, string filePath, string ProcessedInvoicePath)
        {
            ...
            try
            {
                ...
                Regex pattern = new Regex(@"(([0-9])|([0-2][0-9])|([3][0-1]))\-(JAN|FEB|MAR|APR|MAY|JUN|JUL|AUG|SEP|OCT|NOV|DEC)\-\d{2}");
                ...
                int indx1 = pdf.FindIndex(x => x.ToUpper().Contains("Invoice Number".ToUpper()));
                pattern = new Regex(@"\w{3,5}\d{5,7}");
                pdfObject.InvoiceNumber = pdf[indx1 - 2] + pdf[indx1 - 1] + pdf[indx1] + pdf[indx1 + 1] + pdf[indx1 + 2];
                ...
                string PONumber = pdf[indx - 2] + pdf[indx - 1] + pdf[indx] + pdf[indx + 1] + pdf[indx + 2];
                pattern = new Regex(@"[0-9]{9,11}");
                pdfObject.PONumber = pattern.Match(PONumber).Groups[0].Value;
