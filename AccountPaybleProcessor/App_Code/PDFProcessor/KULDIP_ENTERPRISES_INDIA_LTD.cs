...
{
    public class  KULDIP_ENTERPRISES_INDIA_LTD:IPDFProcessor
    {
        ...
        public KULDIP_ENTERPRISES_INDIA_LTD(IGSTDBManager dbManager, TBL_ProcessInstances tblProcessInstance)
        {
        ...
        public  void ExtractPDFData(ViewPdfModel pdfViewModel, string filePath, string ProcessedInvoicePath)
        {
            ...
            try
            {
                ...
                string inv = pdf[kuldipindex - 1] + pdf[kuldipindex] + pdf[kuldipindex + 1] + pdf[kuldipindex + 2];
                pattern = new Regex(@"(\d{3,4})");
                pdfObject.InvoiceNumber = pattern.Match(inv).Groups[0].Value;
